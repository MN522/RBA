...
#define MODULE_SIZE_AUX_WIDTH 8
#define MODULE_SIZE_AUX_HEIGHT 3
#define NUMBER_AUX 10

#define THRESHOLD_CLOCK_PERIOD 17
//#define THRESHOLD_CLOCK_PERIOD 20
CBinding::CBinding()
{
	m_nNodeAdd = 0;
	m_nNodeSub = 0;
...
void CBinding::GetScorePhase1(void)
{
	...
	for (int k = 0; k<NUMBER_AUX; k++) {
		...
		m_FloorplanPro.AddModule(MODULE_SIZE_AUX_WIDTH, MODULE_SIZE_AUX_HEIGHT);
	}
...
void CBinding::GetScorePhase2(void)
{
	...
	m_SAforFloorplanPro.Initialize();
...
void CBinding::ClearCommunicationRequirementPro(void)
{
	COMREQUIREMENT *crptr;
	while(m_listCommunicationRequirementPro){
...
void CBinding::ShowCommunicationRequirement(void)
{
...
#endif
		m_FloorplanPro.GetCoordinateModuleTerminal(crptr->nIndexModuleSource, 1/*InOut*/, &nXs, &nYs);
		m_FloorplanPro.GetCoordinateModuleTerminal(crptr->nIndexModuleDest, 0/*InOut*/, &nXd, &nYd);
		if( crptr->nIndexModuleVia >= 0 ){
			...
			nDelay += m_FloorplanPro.CalcWireDelay(nXv2, nYv2, nXd, nYd);
#ifdef DEBUG_VERBOSE
...
//			TRACE( "Module %d ==> Module %d", crptr->nIndexModuleSource, crptr->nIndexModuleDest );
		...
		ictptr = GetElementInterconnectionTarget();
...
void CBinding::AddRegistersFinal(void)
{
	...
	for( k=0 ; k<N ; k++ ){
		...
		nF = m_aPriority[k];
		nIndexModuleF = GetModuleIndexBoundToNode( nF );
		nLifetimeStart = node[nF].Time+node[nF].C;
		for( eptr=node[nF].olist ; eptr ; eptr=eptr->olist ){
			nT = eptr->t->n;
			nIndexModuleT = GetModuleIndexBoundToNode( nT );
			nLifetimeEnd = node[nT].Time;
